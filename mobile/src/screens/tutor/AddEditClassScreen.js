...
const AddEditClassScreen = ({ route, navigation }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (isEdit) {
        await tutorService.updateClass(classData.id, data);
        Alert.alert('Success', 'Class updated successfully!', [
          { text: 'OK', onPress: () => navigation.goBack() },
        ]);
      } else {
        await tutorService.createClass(data);
        Alert.alert('Request Sent', 'Your class request was sent to the admin for approval.', [
          { text: 'OK', onPress: () => navigation.goBack() },
        ]);
      }
    } catch (error) {
    ...
    >
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        <View style={styles.form}>
          <FormField label="Class Title *" icon="book" placeholder="e.g. Advanced Mathematics O/L" value={title} onChangeText={setTitle} />
          ...
          <FormField label="Schedule *" icon="calendar-outline" placeholder="e.g. Mon & Wed 4:00 PM" value={schedule} onChangeText={setSchedule} />
          <FormField label="Location *" icon="location-outline" placeholder="e.g. Colombo" value={location} onChangeText={setLocation} />
          <FormField label="Price (Rs./month)" icon="cash-outline" placeholder="e.g. 3000 (0 for free)" value={price} onChangeText={setPrice} keyboardType="numeric" />

          <Text style={styles.label}>Description</Text>
          <TextInput
            style={styles.textArea}
            placeholder="Describe your class, teaching method, syllabus coverage..."
          ...
          <TouchableOpacity style={styles.submitButton} onPress={handleSubmit} disabled={loading}>
            {loading ? (
              ...
              <>
                <Ionicons name={isEdit ? 'checkmark-outline' : 'add-circle-outline'} size={20} color={COLORS.white} />
